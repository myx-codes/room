...
export default function AdminBookings() {
  const { t, formatDate, articleStatusLabel, articleCategoryLabel } = useI18n()
  ...
  const toggleArticleStatus = async (article: AdminArticle) => {
    ...
    try {
      ...
      const message = mutationError instanceof Error ? mutationError.message : t('admin.articleUpdateFailed')
  ...
  const removeArticle = async (articleId: string) => {
    ...
    try {
      ...
      const message = mutationError instanceof Error ? mutationError.message : t('admin.articleRemovalFailed')
      setActionError(message)
    } finally {
      setUpdatingArticleId(null)
    }
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="font-display text-2xl font-bold text-foreground">{t('admin.boardArticles')}</h2>
        <p className="text-sm text-muted-foreground mt-1">{t('admin.totalArticles', { count: totalArticles })}</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          ...
          <option value="ACTIVE">{articleStatusLabel('ACTIVE')}</option>
          <option value="WAITING">{articleStatusLabel('WAITING')}</option>
          <option value="HOLD">{articleStatusLabel('HOLD')}</option>
          <option value="DELETE">{articleStatusLabel('DELETE')}</option>
        </select>
        ...
        >
          ...
          <option value="FREE">{articleCategoryLabel('FREE')}</option>
          <option value="NEWS">{articleCategoryLabel('NEWS')}</option>
          <option value="EVENT">{articleCategoryLabel('EVENT')}</option>
          <option value="HELP">{articleCategoryLabel('HELP')}</option>
          <option value="RECOMMEND">{articleCategoryLabel('RECOMMEND')}</option>
          <option value="LIFESTYLE">{articleCategoryLabel('LIFESTYLE')}</option>
        </select>
      </div>

      {actionError && <p className="text-sm text-destructive mb-3">{actionError}</p>}
      {loading && <p className="text-sm text-muted-foreground mb-3">{t('common.loadingArticles')}</p>}
      {error && <p className="text-sm text-destructive mb-3">{t('common.failedToLoadArticles', { message: error.message })}</p>}

      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left">
              <th className="px-6 py-4 font-medium text-muted-foreground">{t('common.articles')}</th>
              <th className="px-6 py-4 font-medium text-muted-foreground hidden sm:table-cell">{t('common.category')}</th>
              <th className="px-6 py-4 font-medium text-muted-foreground hidden md:table-cell">{t('admin.metrics')}</th>
                ...
                <tr key={article._id} className="border-b border-border/50 hover:bg-muted/30 gentle-animation">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatDate(article.createdAt)} · {t('admin.byMember', { id: article.memberId.slice(0, 8) })}
                    </p>
                  </td>

                  <td className="px-6 py-4 hidden sm:table-cell">
                    <span className="bg-muted px-2.5 py-1 rounded-full text-xs font-medium text-foreground">
                      {articleCategoryLabel(article.articleCategory)}
                    </span>
                  </td>

                  <td className="px-6 py-4 hidden md:table-cell text-muted-foreground">
                    {t('admin.likesViewsComments', {
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${cfg.color}`}>
                      ...
                      {articleStatusLabel(article.articleStatus)}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={isUpdating}
                        title={article.articleStatus === 'ACTIVE' ? t('admin.moveToHold') : t('admin.activateArticle')}
                      >
