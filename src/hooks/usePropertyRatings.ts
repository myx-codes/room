...
async function fetchPropertyRatingsFromDatabase(propertyId: string): Promise<PropertyRatingSummary> {
  ...
  let totalPages = 1
  const stars: number[] = []
  const maxPages = 10 // Limit maximum pages to prevent infinite fetching
  ...
  while (page <= totalPages && page <= maxPages) {
    try {
      ...
      console.warn(`Failed to fetch ratings for property ${propertyId}:`, error)
...
export function usePropertyRatings(propertyIds: string[]) {
  ...
  useEffect(() => {
    ...
    const concurrency = Math.min(2, queue.length) // Reduced from 4 to 2
