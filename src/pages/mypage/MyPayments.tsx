...
export default function MyPayments() {
  ...
  const handleAddCard = (e: React.FormEvent) => {
    ...
    if (digits.length < 12 || digits.length > 19) {
      setSubmitError('Card number is invalid.')
      return
    }

    if (!/^(0[1-9]|1[0-2])\/(\d{2})$/.test(cleanExpiry)) {
      setSubmitError('Expiry must be in MM/YY format.')
      return
    }

    if (cleanCvv.length < 3 || cleanCvv.length > 4) {
      setSubmitError('CVV is invalid.')
      return
    }
      ...
      {isAddOpen && (
        <div className="mb-6 max-w-2xl bg-card border border-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display text-lg font-semibold text-foreground">Add Card</h3>
            <button
              type="button"
          ...
          <form onSubmit={handleAddCard} className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              <label className="text-sm text-foreground mb-1.5 block">Expiry</label>
              <input
                value={expiry}
      ...
      <div className="space-y-4 max-w-2xl">
        ...
        {cards.map((card) => (
          <div key={card.id} className="bg-card border border-border rounded-2xl p-5 flex items-center gap-4">
            ...
            <button onClick={() => handleRemoveCard(card.id)} className="text-sm text-destructive hover:underline">Remove</button>
          </div>
        ))}
      </div>
    </div>
